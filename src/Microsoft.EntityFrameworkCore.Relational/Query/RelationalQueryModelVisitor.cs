    ...
    public class RelationalQueryModelVisitor : EntityQueryModelVisitor
    {
        ...
        private class OuterJoinOrderingExtractor : ExpressionVisitor
        {
            ...
            public override Expression Visit(Expression expression)
            {
                ...
                if (binaryExpression != null && !DependentToPrincipalFound)
                {
            ...
            protected override Expression VisitBinary(BinaryExpression node)
            {
                if (node.NodeType == ExpressionType.Equal)
                {
                    ...
                    if (leftProperty != null && rightProperty != null && leftProperty.IsForeignKey() && rightProperty.IsKey())
                    {
                        ...
                        if (matchingForeignKeys.Count() == 1 && rightProperty.GetContainingKeys().Contains(matchingForeignKeys.Single().PrincipalKey))
                        {
                            var matchingForeignKey = matchingForeignKeys.Single();
